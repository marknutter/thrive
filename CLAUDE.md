...
```bash
cp -r .claude/skills/sync-sprintbook   ~/.claude/skills/
cp -r .claude/skills/configure-sso  ~/.claude/skills/
```

**`/sync-sprintbook`** — use from any downstream project to pull Sprintbook improvements in. Runs parallel exploration, diffs both repos, presents a checklist, creates a GH issue and branch.
